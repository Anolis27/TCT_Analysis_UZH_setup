...
This function is not used anymore but was used to make a similar plot as Figure \ref{}. This difference with \texttt{plot$\_$time$\_$resolution$\_$interpad$\_$region$\_$v2()} reside in the selection of the data to compute the jitter. This function was computing the jitter with all the data that was passing trough the noise filter defined in the \texttt{config.py}. This lead to two drawbacks: the filter was to strict for some data at the trench resulting in missing results at low bias voltages, and the projection on the x axis on the plot of \newline \texttt{plot$\_$time$\_$resolution$\_$interpad$\_$region$\_$everything()} was not robust to a tilted sample (trench not completely perpendicular to the x axis).
