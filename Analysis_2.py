...
#test_directory1 = "Data/V2-TW1/120V"
#test_datafile1  = f"{test_directory1}/parsed_from_waveforms.sqlite"
#test_positions1 = f"{test_directory1}/positions.pickle"
...
def get_positions(positions):
    ...
    for i in range(n_position):
        # if x_data[i]*1e6 <= -50 or x_data[i]*1e6 >= 50:
        #     continue
        # if y_data[i]*1e6 <= -50 or y_data[i]*1e6 >= 50:
        #     continue
...
def get_channel_amplitude(datafile, channel, pulse_no = 1):
    ...
    amplitudes = []
    for i in range(n_position) and i>105 and i<1155:
        result = []
        for j in range(n_triggers):
            amplitude = amplitude_data[i,j,1]
            if math.isnan(amplitude) or amplitude > -0.07:
                continue
...
def plot_amplitude_of_one_pad(datafile, channel, pad_positions):
    ...
    for i in range(n_position):
        ...
        for j in range(n_triggers):
            amplitude = amplitude_data[i,j,1]
            if math.isnan(amplitude) or amplitude > -0.07: #or amplitude > 0
                continue
            time_diff = (t_50_data[i,j,2] - t_50_data[i,j,1]) * 1e9
            if time_diff < 98 or time_diff > 99.5:
                continue
...
def plot_2D_separate(datafile, positions): # makes 2D plot of each channel, and projection of one edge
    query_dataset(datafile)
...
def get_pad_positions(datafile, positions, channel): # gives only the central pad positions
    ...
    # ###### OLD VERSION BEGIN #######
    # takes the points_from_edge y from the upper and lower edge??
    # points_from_edge = 15 # this is hardcoded number
    ...
    # # --- NEW: handle empty projections ---
    # if len(edge1_projection) == 0 and len(edge2_projection) == 0:
    #     print(f"[Channel {channel}] No valid amplitude found -> no pad")
    #     return []

    # if len(edge1_projection) == 0:
    #     chosen = "edge2"
    # elif len(edge2_projection) == 0:
    #     chosen = "edge1"
    # else:
    #     chosen = "edge1" if numpy.mean(edge1_projection) < numpy.mean(edge2_projection) else "edge2"
...
def project_onto_y_two_channels(datafile, positions, channel1, channel2, sensor_strip_positions1, sensor_strip_positions2, pdf):
    ...
    for i in sensor_strip_positions:
        for j in range(n_triggers):
            ...
            if math.isnan(amplitude1) or amplitude1 > -0.07:
                amplitude1 = 0
            if time_diff1 < 98 or time_diff1 > 99.5:
                amplitude1 = 0
            if peak_time1 < 4.5 or peak_time1 > 6.5:
                amplitude1 = 0
            if math.isnan(amplitude2) or amplitude2 > -0:
                amplitude2 = 0
            if time_diff2 < 98 or time_diff2 > 99.5:
                amplitude2 = 0
            if peak_time2 < 4.5 or peak_time2 > 6.5:
                amplitude2 = 0
    ...
    for y_position in sorted(amplitudes):
        ...
        bin_min = mu - 4 * std; bin_max = mu + 4 * std
        bin_width = 0.0005 # hardcoded bin width 0.05
        ...
        fig = plt.gcf()
        pdf.savefig(fig, dpi = 100)# , format=pdf)
    return result
...
#                 if peak_time < 4 or peak_time > 6.5:
#                     continue
#                 if y[i] >= -75 and y[i] <= 75: # only pad region
#                     continue
#                 time_differences.append(time_diff)
#     mu, std = statistics.mean(time_differences), statistics.stdev(time_differences)
#     bin_min = mu - 4 * std; bin_max = mu + 4 * std
    ...
    for i in range(n_position):
        for j in range(n_triggers):
            for channel in (chan1, chan2):
                ...
                if peak_time < 4 or peak_time > 6.5:
                    continue
                if y[i] < -75 or y[i] > 75: # only interpad region
                    continue
